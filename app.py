...
class LanguageLearningApp(QWidget):
    def __init__(self, conn, learn_mode=False):
        super().__init__()
        self.conn = conn
    ...
    def init_ui(self):
        ...
        self.item_label = QLabel("Übersetze das Wort ins Englische:")
        ...
        self.next_button.setToolTip("Drücken Sie F2 für das nächste Wort")
    ...
    def load_next_word(self):
        ...
        if self.current_word:
            self.item_label.setText(
                f"Übersetze: '{self.current_word.deutsch}' ins Englische:")
            self.translation_input.clear()
            self.result_label.clear()
            if self.learn_mode:
                self.show_solution_button.setDisabled(False)
        else:
            self.item_label.setText("Keine Wörter verfügbar.")
    ...
    def check_translation(self):
        ...
        if user_input.lower() == self.current_word.englisch.lower():
            self.result_label.setText("Richtig!")
            self.num_correct += 1
        else:
            QMessageBox.critical(self, "Falsche Antwort",
                                f"Falsch. Die korrekte Übersetzung lautet: '{self.current_word.englisch}'")
            self.translation_input.setFocus()
            self.num_errors += 1
    ...
    def show_solution(self):
        if self.current_word:
            self.result_label.setText(
                f"Die richtige Übersetzung lautet: '{self.current_word.englisch}'")
...
class VocabularyApp(QWidget):
    ...
    def start_learning_words(self):
        ...
        self.current_learning_app = LanguageLearningApp(
            self.conn, learn_mode=self.learn_mode)
